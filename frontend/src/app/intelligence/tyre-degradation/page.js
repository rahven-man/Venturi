...
import TyreDegradationForm from "@/components/intelligence/tyreDegradation/TyreDegradationForm";
import TyreDegradationResultView from "@/components/intelligence/tyreDegradation/TyreDegradationResultView";
import TyreDegradationLoadingOverlay from "@/components/intelligence/tyreDegradation/TyreDegradationLoadingOverlay";
import useTyreDegradationModel from "@/hooks/useTyreDegradationModel";

export default function TyreDegradationPage() {
  const { options, loadingOptions, prediction, phase, predicting, error, runPrediction, resetPrediction } = useTyreDegradationModel();
  ...
  if (phase === "revealed" && prediction) {
    return <TyreDegradationResultView prediction={prediction} payload={lastPayload} onReset={resetPrediction} />;
  }

  return (
    <main className="min-h-screen" style={{ background: "#021024" }}>
      <AnalyticsNav active="teams" />
      <TyreDegradationLoadingOverlay active={phase === "loading"} />
      ...
      <section className="relative w-full">
        ...
        <div className="absolute left-6 top-10 max-w-md md:left-14 md:top-16">
          <div className="flex items-center gap-3 text-xs tracking-[0.3em]" style={{ color: "#7da0ca", fontFamily: "var(--font-technical)" }}>
            <span>INTELLIGENCE</span><span>›</span><span style={{ color: "#e10600" }}>TYRE DEGRADATION</span>
          </div>
          <h1 className="mt-4 text-4xl leading-[0.95] md:text-6xl" style={{ color: "#ffffff", fontFamily: "var(--font-display)", fontWeight: 700 }}>
            TYRE DEGRADATION<br /><span style={{ color: "#c1e8ff" }}>PREDICTION</span>
          </h1>
          ...
          <p className="mt-3 text-sm leading-relaxed md:text-base" style={{ color: "#c1e8ff", fontFamily: "var(--font-body)", opacity: 0.85 }}>
            Predict how tyre wear shifts lap pace relative to a fresh-tyre baseline. Build a scenario and let the model do the rest.
          </p>
        </div>
        ...
        <div className="mx-6 mt-6 md:absolute md:right-8 md:top-6 md:bottom-6 md:mx-0 md:mt-0 md:w-[57rem] md:overflow-y-auto lg:right-14 lg:w-[47rem]">
          <TyreDegradationForm options={options} onSubmit={handleSubmit} predicting={predicting} loadingOptions={loadingOptions} />
        </div>
      </section>
