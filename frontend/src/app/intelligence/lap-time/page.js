...
import LapTimeForm from "@/components/intelligence/lapTime/LapTimeForm";
import LapTimeResultView from "@/components/intelligence/lapTime/LapTimeResultView";
import LapTimeLoadingOverlay from "@/components/intelligence/lapTime/LapTimeLoadingOverlay";
import useLapTimeModel from "@/hooks/useLapTimeModel";

export default function LapTimePage() {
  const { options, loadingOptions, prediction, phase, predicting, error, runPrediction, resetPrediction } = useLapTimeModel();
  ...
  if (phase === "revealed" && prediction) {
    return <LapTimeResultView prediction={prediction} payload={lastPayload} onReset={resetPrediction} />;
  }

  return (
    <main className="min-h-screen" style={{ background: "#021024" }}>
      <AnalyticsNav active="teams" />
      <LapTimeLoadingOverlay active={phase === "loading"} />

      <section className="relative w-full">
        <img src="/images/lap-time/LapTimeHero.png" alt="" className="block h-auto w-full select-none" />
        <div
          className="pointer-events-none absolute inset-0"
          style={{ background: "linear-gradient(100deg, rgba(2,16,36,0.85) 0%, rgba(2,16,36,0.5) 38%, rgba(2,16,36,0.2) 58%, rgba(2,16,36,0.75) 100%)" }}
        />
        ...
        <div className="absolute left-6 top-10 max-w-md md:left-14 md:top-16">
          <div className="flex items-center gap-3 text-xs tracking-[0.3em]" style={{ color: "#7da0ca", fontFamily: "var(--font-technical)" }}>
            <span>INTELLIGENCE</span><span>›</span><span style={{ color: "#e10600" }}>LAP TIME</span>
          </div>
          <h1 className="mt-4 text-4xl leading-[0.95] md:text-6xl" style={{ color: "#ffffff", fontFamily: "var(--font-display)", fontWeight: 700 }}>
            LAP TIME<br /><span style={{ color: "#c1e8ff" }}>PREDICTION</span>
          </h1>
          ...
          <p className="mt-3 text-sm leading-relaxed md:text-base" style={{ color: "#c1e8ff", fontFamily: "var(--font-body)", opacity: 0.85 }}>
            Predict lap times using real F1 data and machine learning. Build a race scenario and let the model do the rest.
          </p>
        </div>
        ...
        <div className="mx-6 mt-6 md:absolute md:right-8 md:top-6 md:bottom-6 md:mx-0 md:mt-0 md:w-[57rem] md:overflow-y-auto lg:right-14 lg:w-[47rem]">
          <LapTimeForm options={options} onSubmit={handleSubmit} predicting={predicting} loadingOptions={loadingOptions} />
        </div>
      </section>
