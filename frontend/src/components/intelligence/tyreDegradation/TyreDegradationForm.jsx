...
const initialForm = {
  ...
  lap_number: 20, stint: 2, tyre_life: 8, grid_position: 3,
  current_position: 3, last_lap_time: "",
  is_safety_car_active: false, is_vsc_active: false, is_yellow_flag: false,
  laps_since_green_resumed: 10, laps_since_sc_started: 0, laps_since_vsc_started: 0, sc_deployment_count: 0,
...
export default function TyreDegradationForm({ options, onSubmit, predicting, loadingOptions }) {
  const [form, setForm] = useState(initialForm);
  const [advanced, setAdvanced] = useState(false);
  const available = { drivers: [], teams: [], events: [], compounds: [], ...options };
  const ready = useMemo(() => form.driver && form.team && form.event_name && form.compound, [form]);
  const update = (key, value) => setForm((c) => ({ ...c, [key]: value }));

  function submit(e) {
    e.preventDefault();
    const numeric = ["lap_number", "stint", "tyre_life", "grid_position", "current_position", "laps_since_green_resumed", "laps_since_sc_started", "laps_since_vsc_started", "sc_deployment_count", "air_temp", "track_temp", "humidity", "pressure", "rainfall", "wind_speed", "wind_direction"];
    const payload = { ...form };
    ...
    <form onSubmit={submit} className="w-full max-w-3xl rounded-2xl p-6 md:p-8" style={{ background: "rgba(2,16,36,0.56)", border: "1px solid rgba(193,232,255,0.2)", backdropFilter: "blur(6px)" }}>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "#7da0ca", fontFamily: "var(--font-body)" }}>Set the race context to predict tyre degradation</p>
        </div>
      </div>
        ...
        <div className="mt-4 space-y-6 rounded-lg p-5" style={{ background: "rgba(1,8,23,0.4)", border: "1px solid rgba(193,232,255,0.1)" }}>
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <NumberField label="Laps Since Green" value={form.laps_since_green_resumed} onChange={(v) => update("laps_since_green_resumed", v)} min={0} max={10} />
      ...
      <button disabled={!ready || predicting} className="mt-7 h-14 w-full rounded-md text-sm tracking-[0.2em] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(225,6,0,0.35)] disabled:cursor-not-allowed disabled:opacity-50" style={{ background: "#e10600", color: "#ffffff", fontFamily: "var(--font-technical)" }}>
        {predicting ? "ANALYZING..." : "PREDICT DEGRADATION →"}
      </button>
    </form>
  );
}