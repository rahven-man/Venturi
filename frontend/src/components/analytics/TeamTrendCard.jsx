  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-wrap items-end justify-between gap-5">
        <div>
          <p className="mb-1 text-[10px] tracking-[0.24em] uppercase" style={{ fontFamily: "var(--font-technical)", color: SIGNAL_LIGHT }}>CONSTRUCTOR TRAJECTORY / {activeMode.label}</p>
          <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "1.7rem", color: driverTheme.paleBlue }}>SEASON MOMENTUM</h2>
        </div>
        <div className="flex gap-2">
          {MODES.map((item) => (
            <button
              key={item.key}
              type="button"
              onClick={() => { setMode(item.key); setSelected(null); }}
              className="px-3 py-2 text-[10px] tracking-[0.16em] transition-colors"
              style={{
                fontFamily: "var(--font-technical)",
                color: mode === item.key ? "#ffffff" : driverTheme.paleBlue,
                background: mode === item.key ? SIGNAL_RED : "#021024",
                border: `1px solid ${mode === item.key ? SIGNAL_LIGHT : "rgba(193,232,255,0.2)"}`,
                borderRadius: "6px",
              }}
            >
              {item.label}
      ...
      {data.length < 2 ? (
        <p className="relative z-10 mt-8" style={{ color: driverTheme.skyLight }}>Not enough season data for a trend.</p>
      ) : (
        <>
          <div className="relative z-10 mt-8 overflow-hidden rounded-[14px] p-3" style={{ background: "#021024", border: "1px solid rgba(193,232,255,0.14)", boxShadow: "inset 0 2px 6px rgba(0,0,0,0.6)" }}>
          ...
          <div className="relative z-10 mt-4 min-h-12 rounded-[10px] px-4 py-3" style={{ background: "#021024", border: "1px solid rgba(193,232,255,0.2)", borderLeft: `3px solid ${SIGNAL_RED}` }}>
