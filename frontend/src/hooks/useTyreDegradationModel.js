...
import { getTyreDegradationOptions, predictTyreDegradation } from "@/lib/api";

const LOADING_DURATION_MS = 4600;

export default function useTyreDegradationModel() {
  ...
  async function runPrediction(payload) {
    ...
    try {
      const result = await predictTyreDegradation(payload);
